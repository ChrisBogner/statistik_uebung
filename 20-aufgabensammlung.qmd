...
### Politbarometer 2021: Einlesen von Fremdformaten
Es gibt viele verschiedene Statistikpakete (z. B. SAS, SPSS, Stata), die mit grafischen Oberflächen arbeiten. Da die Analysen darin nicht *reproducible* sind (weil mit der Maus zusammengeklickt), empfehlen wir diese nicht. Nichtsdestotrotz, gibt es manchmal interessante Datensätze, die in den Formaten dieser Statistikpakete vorliegen. ACHTUNG: Diese Aufgabe ist anspruchsvoll!

In dieser Übung lernen Sie das Paket [haven](https://haven.tidyverse.org/) kennen, dass solche Formate einlesen kann. Heaven ist Teil von `tidyverse`, muss aber extra installiert und geladen werden.
...
### Politbarometer 2021: Das Interesse für Politik
Wir analysiere den Datensatz, den Sie in der vorherigen Übung geladen und vorbereitet haben.
...
6. Die Reihenfolge der Merkmalsausprägungen ist unlogisch. Das müssen wir ändern. Bei dieser Variablen gibt es eine logische Reihenfolge: Sehr stark, stark, etwas, kaum, gar nicht, KA. Letzteres steht für keine Angabe. Nutzen Sie den folgenden Code, um die Variable `Polit_interesse` in ein Faktor mit richtiger Reihenfolge der Merkmalsausprägungen umzuwandeln.
```{r}
#| eval: false
...
8. Wir pirschen uns an die relativen Häifigkeiten heran. Was macht der nachfolgende Code. Sehen Sie gegebenenfalls in der Hilfe nach.
```{r}
#| eval: false
gesis_short %>% 
...
Zurück zu unseren Frage: Ist das Interesse der Bürger in allen Bundesländern ähnlich?

9. Beantworten Sie die Frage nun auch graphisch, indem Sie einen Balkendiagramm plotten. Es soll so aussehen:
```{r}
#| echo: false
