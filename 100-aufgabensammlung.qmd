...
[Link zu FelloFish](https://app.fiete.ai/a/cm2iqae202aq741jqkxy2zncf)
...
[Link zu FelloFish](https://app.fiete.ai/a/cm2iqbpoz2aqx41jqyoe1hbyo)
...
2. Laden Sie unter "Downloads/Datasets" (rechts oben) den Datensatz "ZA7856_v1-1-0.dta.zip Stata (Dataset) 1.9 MB" herunter. Dafür werden Sie sich einmalig (und kostenlos) anmelden müssen.
...
```{r}
#| eval: false
gesis <- read_dta('Daten/ZA7856_v1-1-0.dta/ZA7856_v1-1-0.dta')
```
...
[Link zu FelloFish für Feedback](https://app.fiete.ai/a/cm2iqd6xf2arg41jqj69dla1n)

## Exploration von qualitativen Daten
...
[Link zu FelloFish für Feedback](https://app.fiete.ai/a/cm2iqfn3w2at041jqpfru5h9h)

## Exploration von quantitativen Daten
...
[Link zu FelloFish für Feedback](https://app.fiete.ai/a/cm2iqgukg2c783bxtfeqzg8tm)
...
[Link zu FelloFish für Feedback](https://app.fiete.ai/a/cm2iqht9c2au241jquefegqsi)

## Lineare Regression

Wir arbeiten mit den Possum-Daten aus dem Paket `openintro`.

1. Laden Sie das Paket und den Datensatz. Zusätzlich werden sie `tidyverse` und `broom` benötigen.

2. Wir wollen die Kopflängen der Tiere mithilfe der Körperlänge vorhersagen (wie in der Vorlesung). Passen Sie das Modell an.

3. Plotten Sie den Residualplot. Gibt es auffällige Muster im Plot?

4. Sehen Sie sich die Parameter des Models und den Determinationskoeffizienten an. Interpretieren Sie beides.

5. Nun möchten wir die Kopflängen nur mithilfe des Geschlechts der Tiere vorhersagen. Wiederholen Sie die vorherigen Schritte.

6. Vergleichen Sie die beiden Modelle. Welches ist besser? Begründen Sie?

7. Werden die Kopflängen der weiblichen oder der männlichen Tiere besser vorhergesagt? Beantworten Sie diese Frage grafisch mit einem Residualplot. Färben Sie die Residuen sinnvoll ein.

8. Schwierig: Passen Sie ein Modell mit beiden Prädiktoren an. Tipp: Sie können die Prädiktoren mit einem Plus verbinden, nach folgendem Schema: lm(Zielvariable ~ Prädiktor1 + Prädiktor2, data = Datensatz)

9. Vergleichen Sie alle drei Modelle. Welches ist besser?

[Link zu FelloFish für Feedback](https://app.fiete.ai/a/cm2iqixh12c7s3bxthbwe8ywf)
...
[Link zu FelloFish für Feedback](https://app.fiete.ai/a/cm2iqk3962aus41jqiogylh6w)
...
Im Jahr 2013 berichtete die Pew Research Foundation, dass "45% der Erwachsenen in den USA angeben, mit einer oder mehreren chronischen Krankheiten zu leben". Dieser Wert basierte jedoch auf einer Stichprobe, so dass er möglicherweise keine perfekte Schätzung für den interessierenden Parameter der Grundgesamtheit darstellt. Die Studie gab den Standardfehler mit 1.2% an, und die Normalverteilung kann kann in diesem Fall als Modell verwendet werden  [@data:pewdiagnosis:2013]..

a. Berechnen Sie ein 90%-Konfidenzintervall für den Anteil der Erwachsenen in den USA, die mit einer oder mehreren chronischen Krankheiten leben. 
b. Stellen Sie das Konfidenzintervall grafisch dar.
c. Interpretieren Sie das Konfidenzintervall im Zusammenhang mit der Studie.
