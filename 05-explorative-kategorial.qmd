...
Die Funktion `mutate()` kann neue Variablen in einem Datensatz erstellen, verändern oder löschen. In unserem Fall erstellen wir eine neue Variable, die wir `geo_factor` nennen. Die Funktion `as_factor()` wandelt (konvertiert) die Text-Variable `geo` in einen Faktor. Die Funktion `mutate()` fügt neue Variablen am Ende des `tibble`s ein:
```{r}
car_numbers
```
...
Die einzlenen Merkmalsausprägungen sind die verschiedenen Ländern. Der Datensatz enthält `r length(levels(car_numbers$geo_factor))` unterschiedliche Länder.
...
Nun nutzen wir die Funktion `year()` aus `lubridate`, um aus der Variablen `time` nur das Jahr zu extrahieren. Wir erstellen dazu mit `mutate()` wieder einen neue Variable, die wir `time_year` nennen.
```{r}
car_numbers <- car_numbers %>% 
...
In diesem Fall erstellt `mutate()` keine neue Variable, sondern überschreibt (verändert) die vorhandene Variable `time_year`. Das ist möglich und gängige Praxis in R. Jetzt ist `time_year` ein Faktor, was man auch in der Darstelung des `tibble` sehen kann.
```{r}
car_numbers
```
